...
\author{\Large Alberto Prato - University of Padua \\ \href{mailto:albertoprato@studenti.unipd.it}{albertoprato@studenti.unipd.it}}
\date{}
...
\begin{document}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.4\textwidth]{../../Downloads/Flowchart}
	...
	The illustrative test case consists of a solution of 4 massive solute particles (modeled as carbon atoms) embedded in a bath of 11.452 solvent particles (modeled as water). The initial configuration was generated using the \texttt{Packmol} software \cite{packmol}, which distributed the molecules within a cubic simulation box of side length $L = 20.0$ \AA.
	
	To emulate a realistic chemical environment, the Lennard-Jones interaction parameters were adapted from the standard OPLS-AA (for solute) and TIP3P (for solvent) force fields as reported by Jorgensen \textit{et al.} \cite{jorgensen_combined}. The simulation employs a consistent system of units: distances in Ångströms, time in picoseconds, mass in Daltons, and temperature in Kelvin. The specific simulation parameters provided in the \texttt{input.txt} file are listed below:
	\begin{lstlisting}[language=Fortran]
	50000 0.001              ! nk, dt (ps)
	18.015 63.597 3.1507     ! Solvent: m (Dalton), epsilon_OO, sigma_OO
	43.472 3.338             ! Solute: epsilon_CO, sigma_CO (angstrom)
	298.15 0.831             ! Temperature (K), Boltzmann const
	20.0                     ! L_box used in packmol \end{lstlisting}
	
	The system dynamics were propagated for a total duration of $50$ ps, discretized into $50.000$ time steps of $\delta t = 0.001$ ps.
	
	\subsection{Equilibration analysis}\label{sec:equilibration}
	
	The first phase of the analysis concerns the equilibration of the system. As shown in Figure \ref{fig:equilibration}, the transition from the static, energy-minimized initial condition to a dynamic liquid state is monitored via the total potential energy and the MSD.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{../Tesi/Friction-Tensor/Molecular-Dynamics/LJ-2/11452/equilibration}
		\caption{Time evolution of system properties during the initial equilibration phase.}
		\label{fig:equilibration}
	\end{figure}
	
	The upper panel of Fig. \ref{fig:equilibration} illustrates the rapid thermalization of the system. The potential energy rises sharply from the deep minimum of the optimized packing and stabilizes around a stationary mean value within the first few fractions of a picosecond. This indicates that the system has successfully escaped the initial energy minimum. Simultaneously, the bottom panel shows that the MSD exhibits an immediate linear time-dependence, characteristic of the diffusive regime of a liquid, confirming that no lattice-like memory of the initial Packmol configuration remains.
	
	\subsection{Friction tensor analysis}
	
	The molecular dynamics trajectory was analyzed to compute the time-dependent friction tensor. As detailed in Section \ref{sec:friction-tensor}, the current implementation focuses on the diagonal elements of the tensor, $\xi_{ii}^{\alpha \alpha}(t)$.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.8\textwidth]{../Tesi/Friction-Tensor/Molecular-Dynamics/LJ-2/11452/friction-diagonal}
		\caption{Time-dependent memory kernel $\tilde{\xi}(t)$ (diagonal components).}
		\label{fig:friction-diagonal}
	\end{figure}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.8\textwidth]{../Tesi/Friction-Tensor/Molecular-Dynamics/LJ-2/11452/friction-diagonal-zoom}
		\caption{Short-time behavior of the memory kernel $\tilde{\xi}(t)$ (0 - 3 ps).}
		\label{fig:friction-diagonal-zoom}
	\end{figure}
	
	Fig. \ref{fig:friction-diagonal} displays the time evolution of the friction kernel for the four solute particles. To resolve the relevant timescales of the solvent-solute coupling, the analysis focuses on the short-time behavior shown in Fig. \ref{fig:friction-diagonal-zoom}. The memory kernel is characterized by a large initial value at $t=0$. This is followed by a rapid decay occurring within a characteristic timescale of $0.1 - 0.2$ ps. The rapid loss of correlation suggests that for phenomena occurring on timescales $t \gg 0.5$ ps, memory effects become negligible, and the dynamics can be effectively described within the Markovian approximation.
	...
	\begin{enumerate}
		\item Systematically varying the solute geometry to map the friction tensor $\tilde{\bm{\xi}}(\mathbf{R}, t)$ across the conformational landscape.
		\item Implementing the transformation to project the Cartesian friction tensor onto internal coordinates.
	\end{enumerate}
