...
\title{\textbf{User Guide}\\[20pt] \Large Classical Molecular Dynamics \& Friction Tensor Calculation  \vspace{7cm}}
...
\begin{document}
	...
	This Fortran program simulates the classical molecular dynamics of a solute consisting of four spheres immersed in a viscous bath. The simulation uses Lennard-Jones potentials and integrates numerically the equations of motion using the velocity Verlet algorithm.
	...
	\begin{itemize}
		\item GFortran: GNU Fortran compiler.
		\item FFTW3: "Fastest Fourier Transform in the West" library.
	...
	\begin{description}
		...
		Responsible for the evaluation of the forces and the potential energy. It models inter-atomic interactions via the Lennard-Jones potential. The module implements periodic boundary conditions utilizing the minimum image convention and applies a spherical cutoff.
		
		\item[\texttt{minimization\_module.f90}] \hfill \\
		Performs an initial geometric optimization of the solvent structure, utilizing the conjugate gradient method (Polak-Ribière variant) to drive the system towards a local energy minimum.
		
		\item[\texttt{velocity\_init\_module.f90}] \hfill \\
		Initializes the solvent velocities from the Maxwell distribution at a target temperature. The module explicitly removes any net center-of-mass drift resulting from the stochastic sampling.
		
		\item[\texttt{fft\_correlation\_module.f90}] \hfill \\
		A module acting as an interface to the FFTW3 library. It computes time-correlation functions though FFT.
		
		\item[\texttt{friction\_module.f90}] \hfill \\
		It computes the time-dependent memory kernel (friction tensor) via the autocorrelation function of the instantaneous forces exerted by the solvent on the solute.
	...
	The program reads simulation parameters from \texttt{input.txt}. The file format is rigid and parameters must be provided in the exact order shown below.
	...
	\begin{table}[H]
		...
		\begin{tabular}{@{}llp{8cm}@{}}
			...
			1 & \texttt{n}, \texttt{dt} & total number of steps, time step \\[5pt]
			...
			4 & \texttt{temp}, \texttt{k\_B} & temperature, Boltzmann constant \\[5pt]
			5 & \texttt{box\_L} & length of the cubic box \\ \bottomrule
		\end{tabular}
	...
	The initial configuration must be provided in standard XYZ format, typically generated by software like Packmol. The file structure is rigid and must adhere to the specific line order described below.
	
	\begin{table}[H]
		\centering
		\begin{tabular}{@{}llp{8cm}@{}}
			\toprule
			\textbf{Line} & \textbf{Description} \\ \midrule
			1 & total number $N$ of atoms in the system \\[5pt]
			2 & arbitrary string (skipped by the parser) \\[5pt]
			3 -- 6 & coordinates of the 4 solute particles \\[5pt]
			7 -- $N+2$ & coordinates of the remaining solvent particles \\ \bottomrule
		\end{tabular}
		\caption{Structure of \texttt{system.xyz}}
	\end{table}
	
	Example of a \texttt{system.xyz} file:
	\begin{lstlisting}
	267
	Built with Packmol
	C            1.250000        1.510000        0.000000
	C            0.000000        0.670000        0.000000
	C            0.000000       -0.670000        0.000000
	C           -1.250000       -1.510000        0.000000
	O            4.814346      -10.023884        6.353803
	O            1.027902        9.993801        0.817163
	O            2.820898        8.581818       -9.999455
	O            6.655889        6.137787        9.888166
	....\end{lstlisting}
	...
	\begin{itemize}
		\item \textbf{\texttt{trajectory.xyz}}: contains the atomic coordinates of the system at regular intervals (every 100 steps) during the simulation, in the standard XYZ format. This file is compatible with visualization software such as VMD.
